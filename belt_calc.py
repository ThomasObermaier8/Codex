...
def compute_belt_solution(wheels: List[Wheel], prefer_outer: bool = True) -> BeltSolution:
    if len(wheels) < 2:
...
class BeltDesignerApp:
    ...
    def calculate(self) -> None:
        try:
            ...
            alt_mode_name = "Innenkontur" if prefer_outer else "Außenkontur"

            try:
                self.solution = compute_belt_solution(self.wheels, prefer_outer=prefer_outer)
                fallback_used = False
            except ValueError:
                self.solution = compute_belt_solution(self.wheels, prefer_outer=not prefer_outer)
                fallback_used = True
            ...
            lines.append("Riemen")
            if fallback_used:
                lines.append(f"  Gewählte Führung                 : {mode_name} (nicht legbar, auf {alt_mode_name} gewechselt)")
            else:
